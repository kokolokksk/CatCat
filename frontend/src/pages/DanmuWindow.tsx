...
import { EventsOn, WindowSetAlwaysOnTop, WindowSetSize } from '../../wailsjs/runtime/runtime';
...
class DanmuWindow extends React.Component {
  ...
  componentDidMount() {
    ...
    const { muaConfig, allDmList, scList, comeInList, pause ,event} = this.state;
    ...
    this.connectLive();
    (window as any).x = true;
    if(!event && (window as any).x){
      (window as any).x = false;
      this.setState({
        event: true
      })
